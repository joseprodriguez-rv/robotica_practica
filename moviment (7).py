...
class MovimentNode(Node):
    def __init__(self):
        ...
        # FIX #5: guardem la direcció original de l'esquiva i no la toquem durant la maniobra
    ...
    def control_callback(self):
        ...
        if self.estat == 0:
            if self.tipus_obstacle is None:
                ...
                if self.tipus_obstacle == 'PARET':
                    ...
                    # FIX #5: guardem la direcció en iniciar l'esquiva i no la modifiquem més
        ...
        # ── MANIOBRA EN "S" ───────────────────────────────────────────────
            ...
            # FIX #1: fallback de 60 cicles (~6 s) si el LiDAR no veu la paret al lateral
            self.cicles  += 1
            ...
            if gir_prou and (paret_visible or timeout):
                if timeout and not paret_visible:
                    self.get_logger().warn('Estat 1: timeout — paret no visible, continuem igualment')
                else:
            ...
            # FIX #1: fallback de 60 cicles (~6 s) si el LiDAR no detecta la paret al davant
            self.cicles  += 1
            gir_prou      = self.gir_acabat(tolerancia_deg=5.0)
            ...
            if gir_prou and (paret_visible or timeout):
                if timeout and not paret_visible:
                    self.get_logger().warn('Estat 3: timeout — paret no visible, continuem igualment')
                else:
        ...
        # ── ESQUIVAR OBJECTE ──────────────────────────────────────────────
            ...
            # FIX #5: usem _dir_esq_original, no direccio_esquivar (que podria haver canviat)
            graus_obj = 270 if self._dir_esq_original == 1 else 90
            d_obj     = self.distancia_con(graus_obj, finestra=20)
            dist      = self.distancia_recorreguda()
            d_front   = self.distancia_con(0, finestra=20)

            if d_front < 0.20 and dist >= 0.15:
                self.get_logger().warn('Obstacle frontal durant lateral → forçant gir')
                self.estat  = 12
                self.cicles = 0
                self.inici_gir(-90 * self._dir_esq_original)
            elif dist >= 0.25 and d_obj > 0.40:
            ...
            # FIX #5: si hi ha obstacle nou al frontal, girem però sense canviar
            # _dir_esq_original — els estats 14/15/16 han de mantenir la simetria
            if d_front < 0.20 and dist >= 0.10:
                self.get_logger().warn('Obstacle frontal a est.13 → forçant pas a 14')
                self.estat  = 14
                self.cicles = 0
                self.inici_gir(-90 * self._dir_esq_original)
            ...
            dist       = self.distancia_recorreguda()
            d_front_15 = self.distancia_con(0, finestra=20)

            if d_front_15 < 0.20 and dist >= 0.15:
                self.get_logger().warn('Obstacle frontal durant retorn → forçant redreçament')
                self.estat  = 16
                self.cicles = 0
                self.inici_gir(90 * self._dir_esq_original)
            elif (dist >= 0.25 and d_orig > 0.35) or dist >= 0.50:
